...
HdPrmanRenderBuffer::HdPrmanRenderBuffer(SdfPath const& id)
    : HdRenderBuffer(id)
    , _width(0)
    , _height(0)
...
<<<<<<< HEAD:third_party/renderman-24/plugin/hdPrman/renderBuffer.cpp
HdPrmanRenderBuffer::~HdPrmanRenderBuffer()
{
}
=======
HdPrmanRenderBuffer::~HdPrmanRenderBuffer() = default;
>>>>>>> upstream/dev:third_party/renderman-23/plugin/hdxPrman/renderBuffer.cpp
...
/*static*/
size_t
HdPrmanRenderBuffer::_GetBufferSize(GfVec2i const &dims, HdFormat format)
{
...
void
HdPrmanRenderBuffer::Blit(HdFormat format, int width, int height,
                          uint8_t const* data)
{
    // Note: use width (not _width) when indexing into source
    if (width == 0 || height == 0) {
            ...
            for (unsigned int i = 0; i < _width; ++i) {
                ...
                if (convertAsInt) {
                    _ConvertPixel<int32_t>(
                        ...
                        format, &data[(jj * width + ii) * srcPixelSize]);
                } else {
                    _ConvertPixel<float>(
                        _format, static_cast<uint8_t*>(
                            &_buffer[(j * _width + i) * dstPixelSize]),
