...
namespace {
    // Struct for holding instancing info generated by FromPickHit and
    // FromPickHits.
...
_ComputeInstancerAndInstanceIndicesAndLocations(
    ...
    HdContainerDataSourceHandle const &primSource)
{
    _InstanceInfo result({SdfPath(), nullptr, {}, {}});
...
// Implements the bulk of the FromPickHit[s] logic. The infoCache parameter
// can be set to null if no caching is desired. Otherwise the same cache
// object can be passed in to repeated calls to this function to prevent
// duplicate calls to _ComputeInstancerAndInstanceIndicesAndLocations.
static
HdxPrimOriginInfo
_FromPickHitWithCache(HdSceneIndexBaseRefPtr const &sceneIndex,
    ...
    while (true) {
        ...
        if (infoCache) {
            ...
            if (it == infoCache->end()) {
                ...
                infoCache->emplace(path, instanceInfo);
            }
            else {
                instanceInfo = it->second;
            }
        }
        else {
            // Get data from the instancer.
        ...
        primSource = instanceInfo.primSource;
        ...
        const size_t n = instanceInfo.instanceIndices.size();
        if (n == 0) {
            break;
        }
